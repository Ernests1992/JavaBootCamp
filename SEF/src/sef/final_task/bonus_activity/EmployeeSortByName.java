...
public class EmployeeSortByName {
    public static void main(String[] args) throws CustomException {
        ...
        try {
            for (int i = 0; i < employees.length - 1; i++) {
                for (int j = 0; j < employees.length - 1; j++) {
                    int compare = employees[j].getFirstName().compareTo(employees[j + 1].getFirstName());
                    if (compare > 0) {
                        temp = employees[j];
        ...
        System.out.println();
        System.out.println("Employees in alphabetical order (by first name):");
        ...
        try {
            PrintWriter outputFile = new PrintWriter("src\\sef\\final_task\\bonus_activity\\AlphabeticalListOfEmployees.txt");
            outputFile.println("Employees in alphabetical order (by first name):");
